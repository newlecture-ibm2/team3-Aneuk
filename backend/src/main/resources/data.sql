...
-- 대기 중 (PENDING) 요청들
INSERT INTO request (status, priority, department_id, raw_text, summary, confidence, room_id, version, created_at, updated_at)
SELECT 'PENDING', 'NORMAL', 'HK', '새 수건 3장 추가 요청', '수건 교체', 0.95,
       (SELECT id FROM room WHERE number = '1204'), 0, NOW() - INTERVAL '30 minutes', NOW() - INTERVAL '30 minutes'
...
SELECT 'PENDING', 'HIGH', 'HK', '객실 청소 요청드립니다. 외출 후 돌아왔는데 아직 정리가 안 되어있어요.', '객실 청소 요청', 0.92,
       (SELECT id FROM room WHERE number = '501'), 0, NOW() - INTERVAL '15 minutes', NOW() - INTERVAL '15 minutes'
WHERE NOT EXISTS (SELECT 1 FROM request WHERE summary = '객실 청소 요청' AND status = 'PENDING');

-- 진행 중 (IN_PROGRESS) 요청
INSERT INTO request (status, priority, department_id, raw_text, summary, confidence, room_id, assigned_staff_id, version, created_at, updated_at)
SELECT 'IN_PROGRESS', 'HIGH', 'HK', '엑스트라 베드 설치 요청', '침구류 정리', 0.88,
       (SELECT id FROM room WHERE number = '502'),
...
SELECT 'COMPLETED', 'NORMAL', 'HK', '미니바 음료 보충해주세요', '미니바 보충', 0.97,
       (SELECT id FROM room WHERE number = '707'),
       (SELECT id FROM staff WHERE name = '김하우스'),
